...
if (!empty($_POST['galleryImage']) && !empty($_POST['galleryPath'])) {
    if (isset($_FILES['testName'])) {
        print_r('yes');
    }
    $uploaddir = '/var/www/html/images/';
    $imageName = $uploaddir . basename($_FILES['testName']['name']);
    print_r($imageName);
    if (move_uploaded_file($_FILES['testName']['tmp_name'], $imageName)) {
